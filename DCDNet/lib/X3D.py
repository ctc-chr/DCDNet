...
class SubBatchNorm3d(nn.Module):
    ...
    def forward(self, x):
        if self.training and x.shape[0] >= self.num_splits:
            n, c, t, h, w = x.shape
...
class Bottleneck(nn.Module):
    def __init__(self, in_planes, planes, stride=1, downsample=None, index=0, base_bn_splits=8):
        ...
        self.bn1 = SubBatchNorm3d(num_splits=self.base_bn_splits, num_features=planes[0],
                                  affine=True)  # nn.BatchNorm3d(planes[0])
        self.conv2 = conv3x3x3(planes[0], planes[0], stride)
        self.bn2 = SubBatchNorm3d(num_splits=self.base_bn_splits, num_features=planes[0],
                                  affine=True)  # nn.BatchNorm3d(planes[0])
        self.conv3 = conv1x1x1(planes[0], planes[1])
        self.bn3 = SubBatchNorm3d(num_splits=self.base_bn_splits, num_features=planes[1],
                                  affine=True)  # nn.BatchNorm3d(planes[1])
...
class ResNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.bn1 = SubBatchNorm3d(num_splits=self.base_bn_splits, num_features=self.in_planes,
                                  affine=True)  # nn.BatchNorm3d(self.in_planes)
        self.relu = nn.ReLU(inplace=True)
        ...
        self.conv5 = nn.Conv3d(block_inplanes[3][1],
                               ...
                               padding=(0, 0, 0),
                               bias=False)
        self.bn5 = SubBatchNorm3d(num_splits=self.base_bn_splits, num_features=block_inplanes[3][0],
                                  affine=True)  # nn.BatchNorm3d(block_inplanes[3][0])
        if task == 'class':
        ...
        # self.ttsPre2 = TtS(32)
        # self.ttsPre3 = TtS(64)
        # self.ttsPre4 = TtS(128)
        # self.ttsPre2 = TtS(48)
        # self.ttsPre3 = TtS(96)
        # self.ttsPre4 = TtS(192)
        ...
        if stride != 1 or self.in_planes != planes[1]:
            if shortcut_type == 'A':
                ...
                downsample = nn.Sequential(
                    ...
                    SubBatchNorm3d(num_splits=self.base_bn_splits, num_features=planes[1], affine=True)
    ...
    def update_bn_splits_long_cycle(self, long_cycle_bn_scale):
        for m in self.modules():
            if isinstance(m, SubBatchNorm3d):
                ...
                m.split_bn = nn.BatchNorm3d(num_features=m.num_features * m.num_splits, affine=False).to(
                    m.weight.device)
    ...
    def forward(self, x):
        ...
        x = self.layer4(x)
        # x = x + self.ttsPre4(x) * self.alpha[0]
...
def get_inplanes(version):
    planes = {'S': [(36, 16), (72, 32), (144, 64), (288, 128)],
