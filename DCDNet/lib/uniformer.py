...
import os
from model.config_uniformer import params
from modules.TtS_new import TtS

# from UniFormer_main.video_classification.slowfast.utils import logging as logging
...
class CBlock(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        # self.attn = conv_5x5x5(dim, dim, groups=dim)
        # self.attn2_spatial = nn.Conv3d(in_channels=dim, out_channels=dim, kernel_size=(1, 5, 5), padding=(0, 2, 2))
        # self.attn2_time = nn.Conv3d(in_channels=dim, out_channels=dim, kernel_size=(5, 1, 1), padding=(2, 0, 0))
        # self.attn3 = nn.Conv3d(in_channels=dim, out_channels=dim, kernel_size=(5, 5, 5), padding=(2, 4, 4), dilation=(1, 2, 2))
    ...
    def forward(self, x):
        ...
        # x1 = self.attn(x)
        # x2 = self.attn2_spatial(x)
        # x2 = self.attn2_time(x2)
        # x3 = self.attn3(x)
        # x = x1 + x2 + x3
        # x = self.conv2(x)
        # x = x + x_res
...
class SpeicalPatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768):
        ...
        # self.norm_head = nn.BatchNorm3d(in_chans)
...
class Uniformer(nn.Module):
    ...
    def __init__(self):
        ...
        block_3_4_qkv_bias = params['block3_4_qkv_bias']
        qk_scale = params['qk_scale']
        representation_size = params['representation_size']
        drop_rate = params['drop_rate']
        attn_drop_rate = params['atten_drop_rate']
        drop_path_rate = params['drop_path_rate']
        split = params['split']
        std = params['std']
        self.use_checkpoint = params['checkpoint']
        self.checkpoint_num = params['checkpoint_num']
        ...
        if split:
            self.blocks3 = nn.ModuleList([
                SplitSABlock(
                    dim=embed_dim[2], num_heads=num_heads[2], mlp_ratio=mlp_ratio, qkv_bias=block_3_4_qkv_bias, qk_scale=qk_scale,
                    drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i + depth[0] + depth[1]],
                    norm_layer=norm_layer)
                for i in range(depth[2])])
            self.blocks4 = nn.ModuleList([
                SplitSABlock(
                    dim=embed_dim[3], num_heads=num_heads[3], mlp_ratio=mlp_ratio, qkv_bias=block_3_4_qkv_bias, qk_scale=qk_scale,
    ...
    def get_pretrained_model(self):
        if params['pretrain']:
            ...
            checkpoint = torch.load(params['pretrain'])
    ...
    def check_nan_inf(self, x, name):
        a = x.detach().cpu()
        a = a.numpy()
        isnan_list = np.isnan(a)
        isinf_list = np.isinf(a)
        if True in isnan_list or True in isinf_list:
            print(a.shape, name)
            a = np.nan_to_num(a, nan=0.0, posinf=0.0, neginf=0.0)
            a = torch.tensor(a).clone().detach().requires_grad_(True).to('cuda')
            return a
        return x
