...
class VideoDataset(Dataset):
    ...
    def frame_sample_strategy(self, buffer, clip_len):
        frame_total = buffer.shape[0]
        frame_number = frame_total // clip_len
        frame_index = 0
        frame_buffer = np.empty((clip_len, buffer.shape[1], buffer.shape[2], 3), np.dtype('float32'))

        for i in range(0, clip_len - 1):
            time_index = np.random.randint(0, frame_number)
            frame_buffer[i] = buffer[frame_index + time_index, :, :, :]
            frame_index = frame_index + frame_number

        a = frame_total - frame_index
        time_index_1 = np.random.randint(0, a)
        frame_buffer[clip_len - 1] = buffer[frame_index + time_index_1, :, :, :]
    ...
    def crop(self, buffer, clip_len, crop_size):
        ...
        buffer = self.frame_sample_strategy(buffer, clip_len)
        height_index = np.random.randint(buffer.shape[1] - crop_size)
        width_index = np.random.randint(buffer.shape[2] - crop_size)
